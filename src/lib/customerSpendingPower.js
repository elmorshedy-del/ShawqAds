...
export function buildCustomerSpendingPowerAnalysis({
  ...
  orders.forEach((order) => {
    ...
    current.orders += 1;
    if (!current.first_order_at || (order?.created_at && order.created_at < current.first_order_at)) {
      current.first_order_at = order.created_at;
    }
    // Fix the economic-area exposure at the customer's earliest observed paid order.
    // This avoids letting later moves or repeat behavior redefine the acquisition cohort.
    if (!current.postal_code && countryCode && postalCode) {
    ...
    if (countryCode === 'US' && postalCode) {
      ...
      supportedOrderIds.add(String(order?.order_id || `${customerKey}:${order?.created_at || ''}`));
      if (reference.has(postalCode)) {
        matchedOrderIds.add(String(order?.order_id || `${customerKey}:${order?.created_at || ''}`));
  ...
  customers.forEach((customer) => {
    if (customer.country_code !== 'US' || !customer.postal_code) return;
  ...
  const distribution = [1, 2, 3, 4, 5].map((quintile) => {
    ...
    return {
      ...
      revenue_share: safeRatio(revenue, points.reduce((sum, point) => sum + point.lifetime_spend, 0)),
      median_lifetime_spend: median(bucket.map((point) => point.lifetime_spend)),
    };
  });

  const top = distribution.find((row) => row.quintile === 5) || {};
  const matchedCustomerRevenue = points.reduce((sum, point) => sum + point.lifetime_spend, 0);
  const slopePer10k = regression.slope == null ? null : regression.slope * 10000;

  return {
    status: points.length ? 'ready' : 'insufficient_data',
    scope: 'lifetime',
    ...
    methodology: {
      ...
      customer_unit: 'Unique Shopify customer ID; economic area fixed at earliest observed paid order.',
      privacy: 'No customer identifiers, addresses, or postal codes are emitted to the dashboard payload.',
    },
    coverage: {
      ...
      matched_customer_revenue: matchedCustomerRevenue,
      matched_order_revenue: matchedRevenue,
      customer_match_rate: safeRatio(points.length, customers.size),
